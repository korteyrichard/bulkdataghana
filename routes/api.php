...
Route::prefix('v1')->group(function () {
    ...
    // Note: We can't return plain text tokens from database as they're hashed
    // The frontend will rely on local storage for token persistence
    Route::middleware('auth:web')->get('/get-token', function (Request $request) {
        $user = $request->user();
        $hasToken = $user->tokens()->where('name', 'api-token')->exists();
