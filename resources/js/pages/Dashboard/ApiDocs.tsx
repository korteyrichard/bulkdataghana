...
export default function ApiDocs({ auth }: Props) {
    ...
    const fetchExistingToken = async () => {
        // Since we can't retrieve plain text tokens from the server,
        // we'll only check if user has a token but rely on local storage for the actual token
        try {
            ...
            if (response.ok) {
                ...
                // If user has a token on server but not in local storage,
                // they'll need to generate a new one
                if (!data.has_token) {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                setToken(data.token);
                // Save plain text token to local storage
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-4 rounded-md">
                        ...
                        <div className="flex items-center gap-2">
                            <code className="bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-200 px-3 py-2 rounded text-sm font-mono">
                                https://bulkdataghana.com/api/v1
                            </code>
                            <button
                                onClick={() => navigator.clipboard.writeText('https://bulkdataghana.com/api/v1')}
